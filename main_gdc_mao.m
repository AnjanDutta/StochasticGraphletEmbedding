function main_gdc_mao(MAX1, MAX2)
...
file_node_vocab = fullfile(GDCroot,'Data/MAO','node_vocab.mat');
file_edge_vocab = fullfile(GDCroot,'Data/MAO','edge_vocab.mat');
file_hist_indices = fullfile(GDCroot,'Data/MAO','hist_indices.mat');
file_kernels = fullfile(GDCroot,'Data/MAO','kernels.mat');
file_results = fullfile(GDCroot,'Results/MAO','results.txt');
file_diss_matrix = fullfile(GDCroot,'Results/MAO',sprintf('diss_%d_%d.dat',MAX1,MAX2));
file_time = fullfile(GDCroot,'Results/MAO',sprintf('time_%d_%d.txt',MAX1,MAX2));
...
dir_graphs = [GDCroot,'Database/MAO/'];

[train_graph_names,train_classes] = read_mao_cxl([dir_graphs,'train.cxl']);
[valid_graph_names,valid_classes] = read_mao_cxl([dir_graphs,'valid.cxl']);
[test_graph_names,test_classes] = read_mao_cxl([dir_graphs,'mao-test.cxl']);
...
if(~exist(file_hist_indices,'file')||force_compute.hist_indices)
    ...
    for i = 1:ngraphs
        ...
        [~,~,atrvertices,edges,~,atredges] =...
            read_mao_gxl(fullfile(dir_graphs,graph_names{i}));        
        
        classes_vertices = atrvertices;
