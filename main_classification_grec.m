function main_classification_grec()
...
file_node_vocab = fullfile(workhome,'StochasticGraphletEmbedding/SavedData/GREC','node_vocab.mat');
file_edge_vocab = fullfile(workhome,'StochasticGraphletEmbedding/SavedData/GREC','edge_vocab.mat');
% file_kernels = fullfile(workhome,'StochasticGraphletEmbedding/SavedData/GREC','kernels.mat');
...
dir_graphs = [workhome,'Datasets/IAMGraphs/GRECGraph/data'];

[train_graph_names,train_classes] = read_grec_cxl(fullfile(dir_graphs,'train.cxl'));
[valid_graph_names,valid_classes] = read_grec_cxl(fullfile(dir_graphs,'valid.cxl'));
train_graph_names = [train_graph_names; valid_graph_names] ;
train_classes = [train_classes; valid_classes] ;
[test_graph_names,test_classes] = read_grec_cxl(fullfile(dir_graphs,'test.cxl'));
...
for ieps = 1:2
    for idel = 1:2
        ...
        file_hist_indices = fullfile(workhome,'StochasticGraphletEmbedding/SavedData/GREC',sprintf('hist_indices_%d_%d.mat',ieps,idel));
        file_kernels = fullfile(workhome,'StochasticGraphletEmbedding/SavedData/GREC',sprintf('kernels_%d_%d.mat',ieps,idel));
        file_results = fullfile(workhome,'StochasticGraphletEmbedding/SavedData/GREC',sprintf('results_%d_%d.txt',ieps,idel));
